...
int requeteAchatArticle(ARTICLE article, int quantite);
void requeteCancelArticle(int id, int quantite);
  ...
  while(1)
  {
    ...
    switch(m.requete)
    {
      ...
      case ACHAT :
      {
          ...
          // Acces BD
          ARTICLE articleInfo = requeteSqlArticle(m.data1);
          int quantiteAchetee = 0;
          if(articleInfo.stock != -1)
          {
            quantiteAchetee = requeteAchatArticle(articleInfo, atoi(m.data2));
          }
          ...
          reponse.data1 = articleInfo.id;
          strcpy(reponse.data2, articleInfo.intitule);
          sprintf(buffer, "%d", quantiteAchetee);
          strcpy(reponse.data3, buffer);
          strcpy(reponse.data4, articleInfo.image);
          reponse.data5 = articleInfo.prix;
      ...
      case CANCEL:
      {
        ...
        requeteCancelArticle(m.data1, atoi(m.data2));
        break;
      }
...
{   
    ...
    if (article.stock >= quantite)
    {
        ...
        int nouveauStock = article.stock - quantite;
        sprintf(requete, "UPDATE UNIX_FINAL SET stock = %d WHERE id = %d;", nouveauStock, article.id);
...
void requeteCancelArticle(int id, int quantite)
{
    ARTICLE article = requeteSqlArticle(id);
    if(article.stock != -1)
    {
        char requete[256];
        int nouveauStock = article.stock + quantite;
        ...
        if (mysql_query(connexion, requete) != 0)
        {
            fprintf(stderr, "Erreur de mise à jour du stock pour CANCEL: %s\n", mysql_error(connexion));
        }
    }
}
