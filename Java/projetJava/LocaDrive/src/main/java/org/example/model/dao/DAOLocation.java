...
public class DAOLocation implements DataAccessLayer {
    ...
    public boolean deleteContract(Contrat contrat) {
        ...
        boolean result = contracts.remove(contrat.getIdContrat()) != null;
        if (result) {
            saveData(); // Sauvegarder après modification
    ...
    public void saveReservations(List<Reservation> reservationsList) {
        try {
            // D'abord, marquer toutes les voitures comme disponibles
            for (Car car : cars.values()) {
                car.setAvailable(true);
            }
            
            // Mettre à jour la map des réservations avec les nouvelles données
            reservations.clear();
            for (Reservation reservation : reservationsList) {
                reservations.put(reservation.getIdReservation(), reservation);
                
                // Marquer les voitures réservées comme non disponibles
                if (reservation.getCar() != null) {
                    // Si la référence à la voiture est directement disponible
                    ...
                    if (car != null) {
                        ...
                        // Mettre à jour la référence à la voiture dans la réservation avec une copie simplifiée
                        Car simpleCar = new Car();
                        simpleCar.setIdCar(car.getIdCar());
                        simpleCar.setBrand(car.getBrand());
                        simpleCar.setModel(car.getModel());
                        simpleCar.setYear(car.getYear());
                        simpleCar.setPriceday(car.getPriceday());
                        simpleCar.setAvailable(false);
                        
                        reservation.setCar(simpleCar);
                        
                        cars.put(car.getIdCar(), car);
                        System.out.println("Voiture " + car.getIdCar() + " marquée comme non disponible");
                    }
                }
            }
            
            // Sauvegarder les réservations et les voitures dans les fichiers
            ...
            System.out.println("Réservations sauvegardées avec succès. Nombre de réservations : " + reservations.size());
            // Mettre à jour la table des réservations
            if (controller != null) {
                controller.updateAllTables();
            }
